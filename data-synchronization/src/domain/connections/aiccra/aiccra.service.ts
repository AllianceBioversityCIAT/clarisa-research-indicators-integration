...
import { LocElement } from './entities/loc-elements/loc-elements.entity';
import { InstitutionsLocations } from './entities/institutions-locations/institutions-locations.entity';
import { InstitutionClarisaDto } from './complements/dtos/intitution-clarisa.dto';
import { InstitutionsMapper } from './complements/mappers/institutions.mapper';
import { ClrisaMessageDto } from './complements/dtos/clrisa-message.dto';
import { firstValueFrom } from 'rxjs';

@Injectable()
export class AiccraService extends BaseApi {
  private readonly _logger = new LoggerUtil({
    name: AiccraService.name,
  });
  constructor(
    @InjectDataSource('AICCRA') private dataSource: DataSource,
   ...
   */
  async bootstrap() {
    this._logger.verbose(`Start saving data of ${AiccraService.name}`);

    const lastInst = await this.findLastUpdatedInstitution();
    const lastUpdated = lastInst?.updated_at?.getTime() ?? 0;
    ...
    await this.getInstitutions(lastUpdated).then(
      async (res: InstitutionClarisaDto[]) => {
        ...
        for (const el of insertOnly) {
          ...
          saveData.push(InstitutionsMapper(el));
        }
        await this.saveDataFunction(saveData, mss, Institution);
      },
    );
  }
   ...
   */
  private async saveDataFunction<C>(
    saveData: C[],
    mss: ClrisaMessageDto,
    entityClass: new () => C,
  ) {
   ...
   */
  private async findLastUpdatedInstitution(): Promise<Institution | null> {
    return (
      this.dataSource
        .getRepository(Institution)
  ...
  private async getInstitutions(lastUpdatedTime) {
    const baseUrl = 'api/institutions?show=all';
    const fromParam = lastUpdatedTime ? `&from=${lastUpdatedTime}` : '';
    const url = `${baseUrl}${fromParam}`;
    return firstValueFrom(this.getRequest<InstitutionClarisaDto[]>(url)).then(
      (data) => data?.data,
    );
  }
  ...
  async clon_institutionLocations(lastUpdatedTime?: number) {
    ...
    await this.saveDataFunction(toAdd, mss, InstitutionsLocations);
  }
}
